...
const getTicketByCustomId=catchAsync(async(req,res,next)=>{
  const {id} = req.params;

  if(!id){
    return next(new AppError("Provide Ticket ID",400))
  }

  const ticket = await Ticket.findOne({customId:Number(id)});

  if(!ticket){
    return next(new AppError("No Ticket Found with this ID", 400));
  }
...
const setResolutionTime = catchAsync(async (req, res, next) => {
  ...
  ticket.priority = priority || ""; // Default to low if not provided
